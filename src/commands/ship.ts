...
export class ShipCommand extends Command {
	...
	public async messageRun(message: Message, args: Args) {
		const {
			UserOne: { username: firstUsername, memberData: firstMemberData },
			UserTwo: { username: secondUsername, memberData: secondMemberData },
			areBothMembers
		} = await this.getUserData(message, args);
		...
		if (areBothMembers) {
			...
			const heartWidth = heart.width * 0.6; // Scale the heart to 60% of its original size
	...
	private async getUserData(message: Message, args: Args): Promise<{ UserOne: ShipReturnData; UserTwo: ShipReturnData; areBothMembers: boolean }> {
		...
		const fetchMember = async (input: string | undefined): Promise<string | GuildMember | undefined> => {
